  ...
  <section class="card">
    ...
    {:else if references}
      <form onsubmit={create}><div class="form-grid">
        <div class="field"><label for="v2-order-item">완제품</label><select id="v2-order-item" bind:value={createForm.finished_item_id} required><option value=""></option>{#each references.items.filter(item => item.kind === 'finished' && item.active !== false) as item}<option value={String(item.id)}>{item.name ?? item.code}</option>{/each}</select></div>
        ...
        <div class="field"><label for="v2-order-bom">승인 BOM</label><select id="v2-order-bom" bind:value={createForm.bom_revision_id} required><option value=""></option>{#each references.bom_revisions.filter(row => row.status === 'approved' && revisionMatches(row)) as row}<option value={String(row.id)}>{row.revision}</option>{/each}</select></div>
        <div class="field"><label for="v2-order-inspection">검사 기준</label><select id="v2-order-inspection" bind:value={createForm.inspection_revision_id} required><option value=""></option>{#each references.inspection_revisions.filter(row => row.status === 'approved' && revisionMatches(row)) as row}<option value={String(row.id)}>{row.revision}</option>{/each}</select></div>
...
{#if order}<section class="card">
  ...
  {#if availableActions().length}
    <form onsubmit={(event) => { event.preventDefault(); void run(); }}>
      <div class="form-grid">
        <div class="field"><label for="v2-order-action">처리</label><select id="v2-order-action" bind:value={selectedAction} required><option value=""></option>{#each availableActions() as action}<option value={action.action}>{action.label}</option>{/each}</select></div>

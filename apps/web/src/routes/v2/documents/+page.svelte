  ...
  <section class="card">
    ...
    {:else}<form onsubmit={createDraft}>
      <div class="form-grid">
        ...
        {#if kind !== 'component_receipt'}<div class="field"><label for="v2-doc-order">작업 지시</label><select id="v2-doc-order" bind:value={form.order_id} required={['issue','return','material_loss','production','rework','goods_receipt'].includes(kind)}><option value=""></option>{#each activeOrders() as row}<option value={String(row.id)}>{short(row.id)} · {label(row.status)}</option>{/each}</select></div>{/if}
        {#if sourceOptions().length || ['inspection','disposition','rework','goods_receipt'].includes(kind)}<div class="field"><label for="v2-doc-source">원본 문서</label><select id="v2-doc-source" bind:value={form.source_document_id} onchange={sourceChanged} required><option value=""></option>{#each sourceOptions() as source}<option value={String(source.id)}>{label(source.kind)} · {short(source.id)}</option>{/each}</select></div>{/if}
        {#if ['production','rework'].includes(kind)}<div class="field"><label for="v2-doc-session">작업 세션</label><select id="v2-doc-session" bind:value={form.work_session_id}><option value=""></option>{#each sessions.filter(row => row.status === 'active' && (!form.order_id || String(row.order_id) === form.order_id)) as row}<option value={String(row.id)}>{short(row.id)} · {short(row.user_id)}</option>{/each}</select><span class="subtext">현재 서버는 전표 확정 시 세션 연결을 강제 검증하지 않으므로 선택값은 추적 보조 정보입니다.</span></div>{/if}
        ...
        {#if kind === 'disposition'}<div class="field"><label for="v2-doc-disposition">처분</label><select id="v2-doc-disposition" bind:value={form.disposition} required><option value=""></option><option value="disposal">폐기</option><option value="rework">재작업</option></select></div>{/if}
